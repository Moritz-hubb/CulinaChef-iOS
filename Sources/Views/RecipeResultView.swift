...
.fullScreenCover(isPresented: $showCompletion) {
    ...
    private func convertToRecipe() -> Recipe {
        ...
        let categories = plan.categories?.map { $0.capitalized } ?? []
        return Recipe(
            id: UUID().uuidString,
            ...
            difficulty: nil,
            tags: categories.isEmpty ? nil : categories,
            rating: nil
        )
    }
                    ...
                    if let n = plan.nutrition {
                        VStack(alignment: .leading, spacing: 10) {
                            Picker(L.recipe_nährwerte.localized, selection: $nutritionMode) {
                                Text("pro Portion").tag(NutritionMode.perServing)
                                Text("insgesamt").tag(NutritionMode.total)
                            }
...
final class RunningTimer: ObservableObject, Identifiable {
    ...
    init(baseMinutes: Int, label: String) {
        self.baseMinutes = baseMinutes
        self.remaining = baseMinutes * 60
        self.label = label
    }
    ...
    private func playAlarm() {
        // Generate a ringtone pattern: ring-ring-pause
        let sampleRate = 44100.0
        let ringDuration = 0.4  // Each ring
        let pauseDuration = 0.2  // Pause between rings
        let gapDuration = 0.8    // Gap before repeat
        let frequency1 = 880.0   // A5
        let frequency2 = 1046.5  // C6
        let amplitude = 0.5
        
        // Calculate total pattern length
        let ring1Frames = Int(sampleRate * ringDuration)
        let pauseFrames = Int(sampleRate * pauseDuration)
        let ring2Frames = Int(sampleRate * ringDuration)
        let gapFrames = Int(sampleRate * gapDuration)
        let totalFrames = ring1Frames + pauseFrames + ring2Frames + gapFrames
        
        var samples = [Float]()
        
        // First ring (higher pitch)
        for i in 0..<ring1Frames {
            let time = Double(i) / sampleRate
            let envelope = sin(.pi * Double(i) / Double(ring1Frames)) // Fade in/out
            let sample = Float(sin(2.0 * .pi * frequency2 * time) * amplitude * envelope)
            samples.append(sample)
        }
        
        // Pause
        samples.append(contentsOf: [Float](repeating: 0, count: pauseFrames))
        
        // Second ring (lower pitch)
        for i in 0..<ring2Frames {
            let time = Double(i) / sampleRate
            let envelope = sin(.pi * Double(i) / Double(ring2Frames))
            let sample = Float(sin(2.0 * .pi * frequency1 * time) * amplitude * envelope)
            samples.append(sample)
        }
        ...
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("alarm.caf")
